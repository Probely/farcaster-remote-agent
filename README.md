...
1.  Download the most recent Virtual Appliance from the [Releases](https://github.com/Probely/farcaster-remote-agent/releases) page. The VM archive name is `probely-remote-agent-vm-<version>.zip`

1.  Import the OVF file into your virtualization solution

1.  Allocate the required system resources for the Agent VM, as defined in the [System Resources](#system-resources) section
...
To install the script, run the following command on the Remote Agent Virtual Appliance: `sudo ./probely-agent-<id>.run`

1. After running the install script, the Agent should link-up with Probely's cloud.
To make sure that the connection is working properly, run the following command: `sudo probely-farcaster-status`
...
To make sure that the connection is working properly, run `docker logs farcaster-tunnel`. You should see a message similar to `Allocated port [0-9]+ for remote forward to gateway`
...
After Packer finishes building the VM, you should have OVF and VMDK files available on the `output-virtualbox-iso` directory.
...
You can now install the VM using the steps described in the [Virtual machine](#option-1-virtual-machine) installation section.  
