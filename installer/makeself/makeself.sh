...
GPG_EXTRA=""
DU_ARGS=-ks
HEADER=./makeself/makeself-header.sh
TARGETDIR=""
NOOVERWRITE=n
DATE=`LC_ALL=C date`
EXPORT_CONF=n
...
gpg-asymmetric)
    ...
    ;;
openssl)
    GZIP_CMD="openssl enc ${OPENSSL_CIPHER} -pbkdf2 -iter ${OPENSSL_KDF_ITER}"
    GUNZIP_CMD="openssl enc ${OPENSSL_CIPHER} -d -pbkdf2 -iter ${OPENSSL_KDF_ITER}"
